...
def load_mnist(batch_size, is_training=True):
    ...
    if is_training:
        ...
        num_tr_batch = 13500 // batch_size
        num_val_batch = 1479 // batch_size
        ...
        teX = notmnist.test.images
        #fd = open(os.path.join(path, 't10k-labels-idx1-ubyte'))
        #loaded = np.fromfile(file=fd, dtype=np.uint8)
        #teY = loaded[8:].reshape((10000)).astype(np.int32)
        teY = notmnist.test.labels
        num_te_batch = 3745 // batch_size
        return teX , teY, num_te_batch

def load_fashion_mnist(batch_size, is_training=True):
    #path = os.path.join('models', 'data', 'fashion-mnist')
    path = '/home/vinayak/PycharmProjects/Project_notMNIST/CapsLayer/models/data/fashion-mnist/'
    if is_training:
        fd = open(os.path.join(path, 'train-images-idx3-ubyte'))
        loaded = np.fromfile(file=fd, dtype=np.uint8)
        trainX = loaded[16:].reshape((60000, 784)).astype(np.float32)
