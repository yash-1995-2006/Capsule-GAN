...
def routing(input, b_IJ):
    ...
    """changed the num output capsules to 2 from 10 so need to change corresponding values e.g. all 160s to 32 and 10s to 2"""
    ...
    #print input.shape
    #assert input.get_shape() == [cfg.batch_size, 1152, 32, 8, 1]

    u_hat = reduce_sum(W * input, axis=3, keepdims=True)
    u_hat = tf.reshape(u_hat, shape=[-1, 1152, 2, 16, 1])
    #assert u_hat.get_shape() == [cfg.batch_size, 1152, 2, 16, 1]
    ...
    for r_iter in range(cfg.iter_routing):
        with tf.variable_scope('iter_' + str(r_iter)):
            ...
            if r_iter == cfg.iter_routing - 1:
                ...
                #assert s_J.get_shape() == [cfg.batch_size, 1, 2, 16, 1]

                # line 6:
                # squash using Eq.1,
                v_J = squash(s_J)
                #assert v_J.get_shape() == [cfg.batch_size, 1, 2, 16, 1]
                ...
                #assert u_produce_v.get_shape() == [cfg.batch_size, 1152, 2, 1, 1]
